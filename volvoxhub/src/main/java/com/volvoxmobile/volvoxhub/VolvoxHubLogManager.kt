...
object VolvoxHubLogManager {
    ...
    fun log(message: String, level: VolvoxHubLogLevel) {
        if (shouldLog(level)) {
    ...
    private fun shouldLog(level: VolvoxHubLogLevel): Boolean {
        return level == VolvoxHubLogLevel.ERROR || volvoxHubLogLevel == VolvoxHubLogLevel.DEBUG
    }

    private fun printLog(level: VolvoxHubLogLevel, message: String) {
